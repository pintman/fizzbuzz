...
#+BEGIN_SRC python -n
  ...
  file_menu.add_command(label="Exit", command=clicked)

  # adding sub menu
  sub_menubar = tkinter.Menu(menubar)
  menubar.add_cascade(label="submenu", menu=sub_menubar)
  sub_menu = tkinter.Menu(sub_menubar)
  sub_menu.add_command(label="sub exit", command=clicked)
  sub_menubar.add_cascade(label="sub", menu=sub_menu)
...
#+BEGIN_SRC sh -n :results output
  ...
  # Wenn alle geklappt hat, kann der Feature Branch gelöscht werden
...
1883 und 8883 läuft und für die Übertragung von Sensordaten zwischen
Maschinen entwickelt wurde. Ein [[https://www.heise.de/developer/artikel/MQTT-Protokoll-fuer-das-Internet-der-Dinge-2168152.html][heise-Artikel]] beschreibt das Protokoll
ausführlich.
...
** TODO Beispiel mit EA-Modul					   :noexport:

Das folgende Beispiel verwendet das EA-Modul. 

Hierbei agiert ein Taster als Sensor, der beim Drücken in das Topic
=taster0/pressed= eine 1 oder 0 sendet - je nachdem, ob der Taster
gedrückt oder losgelassen wurde. 
...
mqtt_modul -> Broker: subscribe("taster0/pressed/")

ea_modul -> mqt_modul: taster_gedrueckt()
mqtt_modul -> Broker: publish("taster0/pressed/", "1")
Broker -> mqtt_modul: on_message(1)
mqtt_modul -> ea_modul: schalte_led(gruen, False)
mqtt_modul -> ea_modul: schalte_led(gelb, False)
mqtt_modul -> ea_modul: schalte_led(rot, False)
