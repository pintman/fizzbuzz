...
#+RESULTS[7808fd97ac67d268e9761f6bdc4a3978acddc867]:
#+begin_example
fizzbuzz
1
2
...
Wir verwenden zwei Klassen =Spiel= und =FizzbuzzSpiel=, wobei letztere
die erstere erweitert und damit alle Attribute und Methoden von dieser
übernimmt und ggf. um weitere ergänzt. Man kann sagen =FizzbuzzSpiel=
ist ein =Spiel=. Durch die Vererbung muss nicht jedes neue Spiel die
...
diesem Fall spricht man von einem [[https://docs.python.org/3/library/doctest.html][docttest]]. Die Zeichen "=>>>="
...
#=proteced, -=private) und einem /Rückgabewert bzw. Typ/
gekennzeichnet.
...
#+BEGIN_SRC python -n :results output :exports both :cache yes
  class FizzBuzzGame:
      def __init__(self, n):
          # proteced/private attributes marked with one/two underscores
          self.__number = n
...
#+RESULTS[f9da9149f8904a46c326ae2fb0b38601c5d8aaab]:
: Eingabe: 2 Ausgabe: 2
...
* Mit einem neuronalen Netz					   :noexport:

[[http://natureofcode.com/book/chapter-10-neural-networks/][Kapitel 10: Neural Networks]] beschreibt die Funktionsweise eines
...
#+BEGIN_SRC python -n :results output :exports both :cache yes
  ...
  # along http://natureofcode.com/book/chapter-10-neural-networks/

  class Perceptron:
      """
      >>> p = Perceptron(3)
      >>> point = [50,-12,1]
      >>> res = p.feedforward(point)
      """
      def __init__(self, num_inputs):
          self.c = 0.01 # koeffizient, der die Lernrate bestimmt

          # zufällige Werte aus [-1;+1] für die Gewichte.
      ...
      def train(self, inputs, desired, debug=False):
          assert len(inputs) == len(self.weights)
          
          guess = self.feedforward(inputs)
          error = desired - guess
          if debug:
              print("in",inputs, "des", desired, "g", guess, "err", error)

          for i in range(len(self.weights)):
              self.weights[i] += self.c * error * inputs[i]

  class Network:
      def __init__(self, num_ptrons):
          self.ptrons = []
          for i in range(num_ptrons):
              self.ptrons.append(Perceptron(num_ptrons))

      def train(self, inputs, outputs):
          assert len(outputs) == len(self.ptrons)
          
          for pi in len(self.ptrons):
              self.ptrons[pi].train(inputs, outputs[pi])
  ...
  class Trainer:
      """Ein Trainer speichert die Eingabe und die korrekte Antwort."""
      
      def __init__(self, num_inputs, num, a):
  ...
  def fizzbuzz_nn2():
      ...
      nn = Network(max_val + 3)
      for r in range(rounds):
          for i in range(max_val):
              inputs = [0] * max_val
              outputs = [-1] * (max_val+4)            
              inputs[i] = 1
              #print(i, fizzbuzz, len(outputs))
  ...
  def fizzbuzz_nn():
      ...
      # Creating a network of one peceptron per number
      # adding 3 perceptrons for fizz, buzz and fizzbuzz
      #
      # o-- is a perceptron
      # x   fully meshed
      #      
      ...
      #    x  o--  (fizzbuzz) 
      ptrons = []
      for p in range(max_val + 3): # 1-n, fizz, buzz, fizzbuzz
          ptrons.append(Perceptron(max_val))
      print("Anzahl Perceptrons", len(ptrons))        

      # Training the neural network
      for j in range(rounds):
          for i in range(max_val):
              # all inputs are zero except the current number
              inputs = [0] * max_val
              inputs[i] = 1

              # answer a is 1 for the right perceptron, -1 otherwise
              for pi in range(len(ptrons)):
                  if i%3==0 and i%5==0 and pi==fizzbuzz:
                      a = 1
                  elif i%3==0 and pi==fizz:
                      a = 1
                  elif i%5==0 and pi==buzz:
                      a = 1
                  elif i==pi:
                      a = 1
                  else:
                  ...
                  ptrons[pi].train(inputs, a, False)
                  
      # Checking trained network
      for j in range(max_val):
          ...
          print(j, ":", end='')
          for pi in range(len(ptrons)):
              val = ptrons[pi].feedforward(inputs)
              if val == -1:
                  continue
              if pi == fizz:
                  pi="fizz"
...
#+RESULTS[fcd950f6226887b4ffd37fda2465b1331f061747]:
