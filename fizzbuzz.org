...
#+INCLUDE: "~/proj/fizzbuzz/setup.py" src python
...
#+RESULTS:
#+begin_example
Help on module eapi.hw in eapi:
...
CLASSES
    ...
    class DimmbaresEAModul(EAModul)
     ...
     |      >>> ea.cleanup()
     |  
     |  schalte_led(self, led_farbe, helligkeit)
     ...
     |  
     |  ----------------------------------------------------------------------
     |  Methods inherited from EAModul:
     |  
     |  cleanup(self)
     ...
     |  
     |  ----------------------------------------------------------------------
     |  Data descriptors inherited from EAModul:
     |  
     |  __dict__
     ...
     |  
     |  ----------------------------------------------------------------------
     |  Data and other attributes inherited from EAModul:
     |  
     |  LED_GELB = 1
     |  
     |  LED_GRUEN = 2
...
FILE
    /media/marco/2e2c3431-cdb2-4a34-bc6a-f263678c5740/home/marco/proj/fizzbuzz/fizzbuzz-ve/lib/python3.5/site-packages/eapi/hw.py
...
#+INCLUDE: "~/.sopel/fizzbuzz.cfg" src sh
...
#+BEGIN_SRC plantuml :file fizzbuzz-aktdiag.png
@startuml
start
:zahl = 1;
while (zahl <= 20) is (ja)
...
#+BEGIN_SRC plantuml :file fizzbuzz-seqdiag.png
@startuml
Client -> FizzbuzzServer: fizzbuzz(1)
FizzbuzzServer --> Client: 1
Client -> Client: Ausgabe 1
...
Client -> FizzbuzzServer: fizzbuzz(15)
